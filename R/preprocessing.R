...
#' converts units as needed, and removes subjects with missing CACS values.
#'
...
#' @param id_col Optional name of ID column to preserve (e.g., "subject_id", "BioHEART_ID")
#' @param cholesterol_unit Unit of cholesterol values ("mmol/L" or "mg/dL")
#' @param validate Logical, whether to validate data ranges
#' @return Prepared data frame with standardized column names and units (subjects with missing CACS removed)
#' @export
#' @examples
#' \dontrun{
...
#' )
#'
#' # Prepare data with custom ID column
#' prepared_data <- prepare_cohort_data(
#'   raw_data,
#'   id_col = "BioHEART_ID",
#'   cholesterol_unit = "mmol/L"
...
prepare_cohort_data <- function(data,
  ...
  # Add ID column to mapping if specified
  if (!is.null(id_col)) {
    col_mapping <- c(col_mapping, id = id_col)
  }
  ...
  # Add ID column to required columns if specified
  if (!is.null(id_col)) {
  ...
  # Remove subjects with missing CACS
  missing_cacs <- is.na(result$cacs)
  n_missing_cacs <- sum(missing_cacs)

  if (n_missing_cacs > 0) {
    message(sprintf("Removing %d subjects with missing CACS values", n_missing_cacs))
    result <- result[!missing_cacs, , drop = FALSE]
  }

  # Add attributes for tracking
  attr(result, "cholesterol_unit_original") <- cholesterol_unit
  attr(result, "preparation_date") <- Sys.Date()
  attr(result, "n_missing_cacs_removed") <- n_missing_cacs
