...
#' @param class_col Name of classification column (default: "classification")
...
plot_cacs_vs_risk <- function(data,
  ...
  complete_cases <- stats::complete.cases(plot_data$cacs, plot_data$risk_score)
    ...
    if (plot_type == "density") {
      ...
      for (i in seq_along(classes)) {
        if (classes[i] %in% names(densities)) {
          graphics::lines(densities[[classes[i]]],
      ...
      # Histogram
      graphics::hist(plot_data$risk_score,
           main = title,
           xlab = "Average Normalized Risk Score",
           ylab = "Count",
           col = "lightgray",
...
plot_percentile_distribution <- function(data,
  ...
  if (requireNamespace("ggplot2", quietly = TRUE)) {
    ...
    graphics::abline(h = expected_freq, col = "red", lty = 2, lwd = 2)
