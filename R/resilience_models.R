...
fit_cacs_model <- function(data,
  ...
  complete_cases <- stats::complete.cases(cacs_values, risk_scores)
...
calculate_cacs_percentiles <- function(model,
  ...
  tryCatch({
    ...
    count_pred <- stats::predict(model, newdata = pred_data, type = "count")

    # Zero probability predictions (from zero-inflation component)
    zero_prob <- stats::predict(model, newdata = pred_data, type = "zero")

    # Overall mean predictions (combined model)
    mean_pred <- stats::predict(model, newdata = pred_data, type = "response")
...
classify_resilience <- function(percentile_data,
  ...
  # Preserve ID column if it exists in the input data
  if ("id" %in% names(percentile_data)) {
    results$id <- percentile_data$id
  }
  ...
  class_summary <- table(results$classification, useNA = "ifany")
...
model_diagnostics <- function(model, percentile_data = NULL) {
  ...
  fit_stats <- list(
    ...
    aic = stats::AIC(model),
    bic = stats::BIC(model),
    n_observations = length(model$y),  # Use length of response variable
